    ...
    api.on("before_agent_start", async (event: any, ctx: any) => {
      ...
      if (enableL0 && messages && messages.length > 0) {
        ...
        if (repair.repaired) {
          ...
          // ── L0 FIX: Persist repair to disk (JSONL) ──
          // Without this, the corrupted JSONL reloads on restart and the
          // same repair has to happen every boot — eventually bricking
          // the session if L0 can't patch fast enough.
          try {
            const sessionKey = ctx?.sessionKey || "";
            // sessionKey format: "agent:<agentId>:<sessionId>" or similar
            const parts = sessionKey.split(":");
            const agentId = parts.length >= 2 ? parts[1] : "main";

            const sessionsDir = path.join(agentsDir, agentId, "sessions");
            if (fs.existsSync(sessionsDir)) {
              const jsonlFiles = fs.readdirSync(sessionsDir).filter(
                (f: string) =>
                  f.endsWith(".jsonl") &&
                  !f.includes(".backup") &&
                  !f.includes(".deleted") &&
              ...
              for (const jf of jsonlFiles) {
                const fullPath = path.join(sessionsDir, jf);
                // Build synthetic issues from the repaired tool IDs so
                // repairSessionFile knows what to remove from the JSONL
                const syntheticIssues: ValidationIssue[] = repair.removedToolIds.map(
                  (tid) => ({
                    type: "orphaned_tool_result" as const,
                    toolUseId: tid,
                    messageIndex: -1,
                    description: `Error-terminated tool call ${tid} (persisted by L0 runtime repair)`,
                  })
                );

                const diskRepaired = repairSessionFile(fullPath, syntheticIssues, {
                  debug: (msg) => logger.debug?.(msg),
                  info: (msg) => logger.info?.(msg),
                  warn: (msg) => logger.warn(msg),
                  error: (msg) => logger.error(msg),
                ...
                if (diskRepaired) {
                  logger.warn(
                    `[ClawOS L0] ✅ Persisted repair to disk: ${jf} — ` +
                    `removed ${repair.removedToolIds.length} broken tool call(s). ` +
                    `Session will load clean on next restart.`
                  );
                }
              }
            }
          } catch (err) {
            logger.error(`[ClawOS L0] Failed to persist repair to JSONL: ${err}`);
          }
